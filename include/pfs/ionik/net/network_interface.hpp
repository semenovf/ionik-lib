...
//
// Changelog:
//      2021.06.24 Initial version (netty-lib).
...
struct network_interface_data
{
    ...
    //inet4_addr ip4;
    std::uint32_t ip4;
    string_type ip4_name;

    // IPv6 address associated with interface
    string_type ip6_name;
...
public:
    ...
    std::uint32_t ip4_addr () const noexcept
    {
        return _data.ip4;
    }

    string_type ip4_name () const noexcept
    {
        return _data.ip4_name;
    }

    string_type ip6_name () const noexcept
    {
        return _data.ip6_name;
    }
...
IONIK__EXPORT std::string to_string (network_interface_type type);
IONIK__EXPORT std::string to_string (network_interface_status status);
